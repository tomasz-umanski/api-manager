...
export function AuthPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError('Invalid email or password. Check Firebase credentials and try again.')
    } finally {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-canvas px-4 text-zinc-100">
      ...
      <form onSubmit={onSubmit} className="panel w-full max-w-[445px] p-6">
        ...
        <p className="mt-1 text-sm text-muted">Sign in with Firebase Authentication</p>
        ...
        <div className="relative mt-2">
          ...
          <TextInput id="email" className="pl-10" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} />
        </div>
        ...
        <div className="relative mt-2">
          ...
          <TextInput id="password" className="pl-10 pr-10" type="password" required minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} />
          <Eye className="absolute right-3 top-1/2 -translate-y-1/2 text-muted" size={16} />
        </div>

        {error ? <p className="mt-4 rounded-tool border border-danger/30 bg-danger/10 px-3 py-2 text-sm text-danger">{error}</p> : null}
        ...
        <Button type="button" variant="ghost" className="w-full normal-case" disabled={isSubmitting} onClick={() => void onRegister()}>
